...
The simplest form of text formatting can be achieved in \LaTeX \ as follows:
\begin{paracol}{2}
\switchcolumn[0]
Code
\begin{lstlisting}[language={[LaTeX]TEX}]
...
\newpage
\subsection{Tables}
Tables are common in almost all kinds of documents. \LaTeX provides a large set
of tools to customize tables.
The simplest example for a table would be as follows: \\
In this example we will use a \textit{tabular}.\\
...
% ---- table example 2 ------- %
adding borders can be achieved as follows:
\begin{lstlisting}[language={[LaTeX]TEX}]
\begin{center}
\begin{tabular}{ |c|c|c| }
 cell1 & cell2 & cell3 \\
...
% ---- table example 2 ------- %
Fixed length tables
\todo{fixed lengeth table. Rendering failed. fix code}
\begin{lstlisting}[language={[LaTeX]TEX}]
\begin{center}
...
\subsection{images}
In order to use iamges in \LaTeX, we need to include a package called graphicx.

\begin{lstlisting}[language={[LaTeX]TEX}]
% to use graphicx we nned to include it
\usepackage{graphicx}
% set graphicx path
\graphicspath( {images} )
\end{lstlisting}
...
\LaTeX allows the user to spcify certain global options about the document.\\
...
\item Landscape print mode
\item Single and double sided documents
\item Titlepage behavior
\item Chapter opening page
\end{itemize}
A document in \LaTeX can inheret traits from a class of documents. We will talk list 
some of the most common documents classes.

\subsection{Books}
A book is one of the most used classes of documents in \LaTeX. The book document comes with a 
number of global options that allow for customization of certain elements of the document.
